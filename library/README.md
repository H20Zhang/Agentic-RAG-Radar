...
| 问题 | 入口 | 当前核心张力 |
|---|---|---|
| **Planning & Query Formulation** | [进入](../categories/planning-query-formulation.md) | 哪些 search behavior 可以在 evidence 到来前 compile，哪些必须在线决定？ |
| **Retrieval & Tool Use** | [进入](../categories/retrieval-tool-use.md) | opaque top-k 够不够，还是需要 search/read/filter/navigation 等显式 interface？ |
| **Iterative Reasoning & Verification** | [进入](../categories/iterative-reasoning-verification.md) | 什么时候继续、重写、回退、停止？什么 progress signal 才有用？ |
| **Learning & Optimization** | [进入](../categories/learning-optimization.md) | retrieval/control policy 如何从 rollout、feedback 或 persistent state 中学到？ |
| **Evaluation & Analysis** | [进入](../categories/evaluation-analysis.md) | backend、interface、harness、model、budget 怎么分开，才能做 causal attribution？ |
| **Multi-Agent & Orchestration** | [进入](../categories/multi-agent-orchestration.md) | 多个 search/reasoning actor 的并行与 coordination 什么时候真的有必要？ |
