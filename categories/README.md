...
| [Retrieval & Tool Use](retrieval-tool-use.md) | What corpus boundary, evidence operations, source structure, and resource semantics should the agent control? | DCI exposes interface resolution; RISE/DR-DCI bound it for scale; RARG guides local interaction; SIEVE factorizes admissibility, ranking, inspection, and reading. |
...
The interface correction is important. A conventional retriever, raw direct-corpus interaction, a bounded persistent workspace, and a structured search-inspect-fetch interface expose different evidence operations even if they use the same answer model. SIEVE further shows that **candidate admissibility, ranking, inspection surface, and read granularity** should not be collapsed into one “retriever” variable.

The harness correction is separate: identical retrieval can still behave differently when tool outputs are injected inline versus made available as files or when prompts/tool contracts change.

Two additional stress tests remain. **State can be editable**, not only descriptive: rollback semantics should be separated from the policy deciding when to use them. And **the training distribution can self-degrade**: SearchMaster shows that self-play needs explicit controls against pseudo multi-hop tasks, shallow difficulty signals, and tool-use drift.
