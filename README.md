...
**状态：** 最后更新：**2026-08-28** · 最后合成：**2026-08-28T01:27:38Z（UTC）**
...
<details><summary>2026-08-28 · RetrievalRouter · Adaptivity placement → query-conditioned pipeline routing <!-- timefirst:area=query-conditioned-pipeline-routing --> — 在看到证据前，根据 query 选择 retrieval modality 与 architecture。 <!-- timefirst:delta=query-conditioned-pipeline-routing --></summary>

**问题。** retrieval modality 与 architecture 能否从部署时常量变成 query-conditioned policy decision？ <!-- timefirst:question=query-conditioned-retrieval-pipeline-selection -->

**证据。** soft-target adaptive baseline：`λ=.1` 时 RetrievalRouter 为 **0.755 nDCG@5 / 0.666s**，adapted hard-label router 为 **0.715 / 0.476s**，best static ML 为 **0.737 / 8.283s**。 <!-- timefirst:evidence=retrievalrouter-controlled~soft-target-adaptive-baseline -->

**限制。** multi-index lifecycle cost：在线 latency 没有包含同时维护四套 neural index + BM25 的 build、update、storage 与 cache pressure。 <!-- timefirst:caveat=retrievalrouter-boundary~multi-index-lifecycle-cost -->

**地图。** `early_signal`：retrieval-substrate choice 是值得单独比较的控制面，但完整生命周期 economics 尚未定价。

**链接。** [RetrievalRouter: Joint Modality and Architecture Selection for Document Retrieval](https://arxiv.org/abs/2608.25625) · [代码](https://github.com/emrekuruu/retrieval-router) · [英文深读](papers/2608.25625.md) · [中文深读](papers/2608.25625.zh.md)

</details>

<a id="entry-2608.25115"></a>
<details><summary>2026-08-28 · PACE · Resource accounting → serving-pressure retrieval budgeting <!-- timefirst:area=serving-pressure-retrieval-budgeting --> — 先对固定 evidence pool 重排，再随 reranker 与 LLM 的压力变化调整 reranking depth。 <!-- timefirst:delta=serving-pressure-retrieval-budgeting --></summary>

**问题。** RAG 能否在负载上升时少买 reranking compute，同时不丢掉下游需要的 evidence？ <!-- timefirst:question=serving-pressure-reranking-budget -->

**证据。** same adaptive-budget policy：HotpotQA complete R@5 中，PACE `D=50` 为 **62.38**，standard dense `D=100` 为 **41.13**；QPS=1.8 时约用一半 full budget，R@5 仍高约 20%。 <!-- timefirst:evidence=pace-controlled~same-adaptive-budget-policy -->

**限制。** candidate-pool conditioning：HotpotQA/2Wiki 只保留 top-100 已含完整 gold evidence 的 query，online headline 主要是 evidence recall + serving latency，而非 matched answer correctness。 <!-- timefirst:caveat=pace-boundary~candidate-pool-conditioning -->

**地图。** `early_signal`：system pressure 可以进入 adaptivity surface，但 semantic ordering 与 queue control 仍需要 factorial end-to-end 对照。

**链接。** [Less can be More: Relieving RAG Bottlenecks via Evidence Frontloading and Pressure-Adaptive Budgeting](https://arxiv.org/abs/2608.25115) · [英文深读](papers/2608.25115.md) · [中文深读](papers/2608.25115.zh.md)

</details>

<a id="entry-2608.25037"></a>
<details><summary>2026-08-28 · Retrieve, Match, Escalate · Resource accounting → cost-tiered hard-tail escalation <!-- timefirst:area=cost-tiered-hard-tail-escalation --> — 只把 ambiguous product-linking cases 升级到 multimodal external-evidence acquisition。 <!-- timefirst:delta=cost-tiered-hard-tail-escalation --></summary>

**问题。** 哪些 case 值得为昂贵的 multimodal Web-search stage 付费，而不是便宜地 accept/reject？ <!-- timefirst:question=confidence-gated-external-evidence-escalation -->

**证据。** production hard-tail coverage：约 200 万 records 上，automatic coverage **68.1% → 77.1%**；另一组 equal-precision audit 中 open agent recall **88% vs 92%**，per-pair cost 约为 closed stack 的七分之一。 <!-- timefirst:evidence=rme-controlled~production-hard-tail-coverage -->

**限制。** unmatched no-search control：production uplift 使用 previous-generation cheap linker，Stage 3 也没有 same-VLM no-Web-search ablation。 <!-- timefirst:caveat=rme-boundary~unmatched-no-search-control -->

**地图。** `early_signal`：selective escalation 获得 production cost/coverage 证据，但 Web search 本身尚未被因果隔离。

**链接。** [Retrieve, Match, Escalate: Accurate and Scalable Product Linking with VLM-Distilled Cross-Encoders and Agentic VLMs](https://arxiv.org/abs/2608.25037) · [英文深读](papers/2608.25037.md) · [中文深读](papers/2608.25037.zh.md)
...
- **`new_signal` · Query conditioned pipeline routing · retrieval modality 与 architecture 成为 query-level policy choice。** <!-- timefirst:direction key="query-conditioned-pipeline-routing" state="new_signal" supports="2608.25625" confidence="medium" implication="price-multi-index-lifecycle-before-routing-credit" timing="radar_published_at" synthesized="2026-08-28T01:27:38Z" prior="none" -->
  支撑：[RetrievalRouter](#entry-2608.25625)；置信度：**medium**；时间依据：`radar_published_at`；精确合成时间：`2026-08-28T01:27:38Z`（UTC）；研究设计含义（price multi index lifecycle before routing credit）：把 index build、update、storage、cache residency、training 与 online latency 全部计价后，再与 single-index 和 cascade 比较 routing；先验地图证据：`none`。

- **`new_signal` · Serving pressure retrieval budgeting · runtime serving pressure 控制本次购买多少 reranking compute。** <!-- timefirst:direction key="serving-pressure-retrieval-budgeting" state="new_signal" supports="2608.25115" confidence="medium" implication="separate-evidence-ordering-from-runtime-budget-control" timing="radar_published_at" synthesized="2026-08-28T01:27:38Z" prior="none" -->
  支撑：[PACE](#entry-2608.25115)；置信度：**medium**；时间依据：`radar_published_at`；精确合成时间：`2026-08-28T01:27:38Z`（UTC）；研究设计含义（separate evidence ordering from runtime budget control）：在同一 retriever、reranker、generator、hardware 与 arrival trace 下做 frontloading on/off × adaptive D on/off；先验地图证据：`none`。

- **`new_signal` · Cost tiered hard tail escalation · ambiguous case 可被路由到更丰富的 evidence environment。** <!-- timefirst:direction key="cost-tiered-hard-tail-escalation" state="new_signal" supports="2608.25037" confidence="medium" implication="match-hard-tail-model-and-search-before-crediting-web-acquisition" timing="radar_published_at" synthesized="2026-08-28T01:27:38Z" prior="none" -->
  支撑：[Retrieve, Match, Escalate](#entry-2608.25037)；置信度：**medium**；时间依据：`radar_published_at`；精确合成时间：`2026-08-28T01:27:38Z`（UTC）；研究设计含义（match hard tail model and search before crediting web acquisition）：固定 retrieval、confidence band、VLM、images 与 prompts，再比较 no-search、fixed-search 与 adaptive-search Stage 3；先验地图证据：`none`。
...
- **`new_signal` · Span grounded evidence construction · verified spans 被物化为 claim-level support/conflict state。** <!-- timefirst:direction key="span-grounded-evidence-construction" state="new_signal" supports="2608.24667" confidence="medium" implication="separate-retrieval-recording-and-stopping-under-matched-budget" timing="radar_published_at" synthesized="2026-08-28T01:27:38Z" prior="none" -->
  支撑：[EviGraph](#entry-2608.24667)；置信度：**medium**；时间依据：`radar_published_at`；精确合成时间：`2026-08-28T01:27:38Z`（UTC）；研究设计含义（separate retrieval recording and stopping under matched budget）：固定 raw results、verifier、answer model 与预算，独立改变 evidence representation、recording 与 stopping；先验地图证据：`none`。
...
- **`new_signal` · Structurally bounded exploration · domain structure 可预先界定 candidate space 与 stopping。** <!-- timefirst:direction key="structurally-bounded-exploration" state="new_signal" supports="2608.24809" confidence="medium" implication="compare-bounded-and-open-search-under-one-output-contract" timing="radar_published_at" synthesized="2026-08-28T01:27:38Z" prior="none" -->
  支撑：[Crase](#entry-2608.24809)；置信度：**medium**；时间依据：`radar_published_at`；精确合成时间：`2026-08-28T01:27:38Z`（UTC）；研究设计含义（compare bounded and open search under one output contract）：固定 corpus、seed retriever、generator、output contract 与预算，比较 fixed bound、adaptive expansion 与 open-ended search；先验地图证据：`none`。
...
- **`reinforced` · Evidence path operation surfaces · 显式 evidence-path operations 获得跨任务支撑。** <!-- timefirst:direction key="evidence-path-operation-surfaces" state="reinforced" supports="2608.17889,2608.18613" confidence="medium" implication="make-evidence-path-operations-explicit" timing="radar_published_at" synthesized="2026-08-28T01:27:38Z" prior="field-map" -->
  支撑：[VisDocAgentBench](#entry-2608.17889) · [CTIFoundry](#entry-2608.18613)；置信度：**medium**；时间依据：`radar_published_at`；精确合成时间：`2026-08-28T01:27:38Z`（UTC）；研究设计含义（make evidence path operations explicit）：在共享 output contract 下显式暴露 search / resolve / traverse / inspect / read，并以匹配 backend、harness 与预算的静态对照检验；先验地图证据：[Interface resolution](#field-map)。

- **`reinforced` · Supersession aware state assembly · 历史保留与 operative-state resolution 可分别放在 answer time 和 write time。** <!-- timefirst:direction key="supersession-aware-state-assembly" state="reinforced" supports="2608.19652,2608.23265" confidence="medium" implication="compare-answer-time-and-write-time-validity" timing="radar_published_at" synthesized="2026-08-28T01:27:38Z" prior="field-map" -->
  支撑：[StateMem](#entry-2608.19652) · [EvoWiki](#entry-2608.23265)；置信度：**medium**；时间依据：`radar_published_at`；精确合成时间：`2026-08-28T01:27:38Z`（UTC）；研究设计含义（compare answer time and write time validity）：在 history access、extraction、answer context、update cost 与 dependency handling 匹配时移动 supersession resolution；先验地图证据：[State persistence](#field-map)。
