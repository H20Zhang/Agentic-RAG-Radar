...
**Status:** Last updated: **2026-08-28** · Last synthesized: **2026-08-28T01:27:38Z (UTC)**
...
<a id="entry-2608.25625"></a>
<details><summary>2026-08-28 · RetrievalRouter · Adaptivity placement → query-conditioned pipeline routing <!-- timefirst:area=query-conditioned-pipeline-routing --> — Chooses retrieval modality and architecture from the query before evidence acquisition. <!-- timefirst:delta=query-conditioned-pipeline-routing --></summary>

**Question.** Can retrieval modality and architecture become a query-conditioned policy decision rather than a deployment-time constant? <!-- timefirst:question=query-conditioned-retrieval-pipeline-selection -->

**Evidence.** Soft-target adaptive baseline: at `λ=.1`, RetrievalRouter reaches **0.755 nDCG@5 / 0.666s** versus adapted hard-label routing **0.715 / 0.476s** and best static ML **0.737 / 8.283s**. <!-- timefirst:evidence=retrievalrouter-controlled~soft-target-adaptive-baseline -->

**Caveat.** Multi-index lifecycle cost: online latency excludes build, update, storage, and cache pressure from keeping four neural indexes plus BM25 available. <!-- timefirst:caveat=retrievalrouter-boundary~multi-index-lifecycle-cost -->

**Map.** `early_signal`: retrieval-substrate choice is a useful control surface, but full lifecycle economics remain unpriced.

**Links.** [RetrievalRouter: Joint Modality and Architecture Selection for Document Retrieval](https://arxiv.org/abs/2608.25625) · [Code](https://github.com/emrekuruu/retrieval-router) · [English deep note](papers/2608.25625.md) · [中文深读](papers/2608.25625.zh.md)

</details>

<a id="entry-2608.25115"></a>
<details><summary>2026-08-28 · PACE · Resource accounting → serving-pressure retrieval budgeting <!-- timefirst:area=serving-pressure-retrieval-budgeting --> — Reorders a fixed evidence pool, then changes reranking depth as reranker versus LLM pressure moves. <!-- timefirst:delta=serving-pressure-retrieval-budgeting --></summary>

**Question.** Can a RAG system spend less reranking compute under load without discarding the evidence needed downstream? <!-- timefirst:question=serving-pressure-reranking-budget -->

**Evidence.** Same adaptive-budget policy: HotpotQA complete R@5 is **62.38** at PACE `D=50` versus **41.13** for standard dense `D=100`; at QPS 1.8 it uses roughly half the full budget while reporting about 20% higher R@5. <!-- timefirst:evidence=pace-controlled~same-adaptive-budget-policy -->

**Caveat.** Candidate-pool conditioning: HotpotQA/2Wiki retain queries whose complete gold evidence already appears in top-100, and the online headline is evidence recall plus serving latency rather than matched answer correctness. <!-- timefirst:caveat=pace-boundary~candidate-pool-conditioning -->

**Map.** `early_signal`: system pressure is a legitimate adaptivity input, but semantic ordering and queue control still need a factorial end-to-end test.

**Links.** [Less can be More: Relieving RAG Bottlenecks via Evidence Frontloading and Pressure-Adaptive Budgeting](https://arxiv.org/abs/2608.25115) · [English deep note](papers/2608.25115.md) · [中文深读](papers/2608.25115.zh.md)

</details>

<a id="entry-2608.25037"></a>
<details><summary>2026-08-28 · Retrieve, Match, Escalate · Resource accounting → cost-tiered hard-tail escalation <!-- timefirst:area=cost-tiered-hard-tail-escalation --> — Sends only ambiguous product-linking cases into multimodal external-evidence acquisition. <!-- timefirst:delta=cost-tiered-hard-tail-escalation --></summary>

**Question.** Which cases should pay for a costly multimodal Web-search stage instead of being resolved or rejected cheaply? <!-- timefirst:question=confidence-gated-external-evidence-escalation -->

**Evidence.** Production hard-tail coverage: on roughly 2M records, automatic coverage moves **68.1% → 77.1%** after escalation; a separate equal-precision audit reports open-agent recall **88% vs 92%** for the closed stack at about one-seventh the per-pair cost. <!-- timefirst:evidence=rme-controlled~production-hard-tail-coverage -->

**Caveat.** Unmatched no-search control: the production uplift uses a previous-generation cheap linker, and Stage 3 lacks a same-VLM no-Web-search ablation. <!-- timefirst:caveat=rme-boundary~unmatched-no-search-control -->

**Map.** `early_signal`: selective escalation is supported as a production cost/coverage control surface; Web search itself is not causally isolated.

**Links.** [Retrieve, Match, Escalate: Accurate and Scalable Product Linking with VLM-Distilled Cross-Encoders and Agentic VLMs](https://arxiv.org/abs/2608.25037) · [English deep note](papers/2608.25037.md) · [中文深读](papers/2608.25037.zh.md)
...
- **`new_signal` · Query conditioned pipeline routing · Retrieval modality and architecture become a query-level policy choice.** <!-- timefirst:direction key="query-conditioned-pipeline-routing" state="new_signal" supports="2608.25625" confidence="medium" implication="price-multi-index-lifecycle-before-routing-credit" timing="radar_published_at" synthesized="2026-08-28T01:27:38Z" prior="none" -->
  Supports: [RetrievalRouter](#entry-2608.25625); confidence: **medium**; timing basis: `radar_published_at`; Exact synthesis time: `2026-08-28T01:27:38Z` (UTC); Research-design implication (price multi index lifecycle before routing credit): compare routing against single-index and cascade controls after charging index build, update, storage, cache residency, training, and online latency; prior map evidence: `none`.

- **`new_signal` · Serving pressure retrieval budgeting · Runtime serving pressure controls how much reranking compute to buy.** <!-- timefirst:direction key="serving-pressure-retrieval-budgeting" state="new_signal" supports="2608.25115" confidence="medium" implication="separate-evidence-ordering-from-runtime-budget-control" timing="radar_published_at" synthesized="2026-08-28T01:27:38Z" prior="none" -->
  Supports: [PACE](#entry-2608.25115); confidence: **medium**; timing basis: `radar_published_at`; Exact synthesis time: `2026-08-28T01:27:38Z` (UTC); Research-design implication (separate evidence ordering from runtime budget control): cross frontloading on/off with adaptive D on/off under the same retriever, reranker, generator, hardware, and arrival trace; prior map evidence: `none`.

- **`new_signal` · Cost tiered hard tail escalation · Ambiguous cases can be routed into a richer evidence environment.** <!-- timefirst:direction key="cost-tiered-hard-tail-escalation" state="new_signal" supports="2608.25037" confidence="medium" implication="match-hard-tail-model-and-search-before-crediting-web-acquisition" timing="radar_published_at" synthesized="2026-08-28T01:27:38Z" prior="none" -->
  Supports: [Retrieve, Match, Escalate](#entry-2608.25037); confidence: **medium**; timing basis: `radar_published_at`; Exact synthesis time: `2026-08-28T01:27:38Z` (UTC); Research-design implication (match hard tail model and search before crediting web acquisition): hold retrieval, confidence band, VLM, images, and prompts fixed while comparing no-search, fixed-search, and adaptive-search Stage 3; prior map evidence: `none`.

- **`new_signal` · Span grounded evidence construction · Verified spans become claim-level support/conflict state.** <!-- timefirst:direction key="span-grounded-evidence-construction" state="new_signal" supports="2608.24667" confidence="medium" implication="separate-retrieval-recording-and-stopping-under-matched-budget" timing="radar_published_at" synthesized="2026-08-28T01:27:38Z" prior="none" -->
  Supports: [EviGraph](#entry-2608.24667); confidence: **medium**; timing basis: `radar_published_at`; Exact synthesis time: `2026-08-28T01:27:38Z` (UTC); Research-design implication (separate retrieval recording and stopping under matched budget): hold raw results, verifier, answer model, and budget fixed while varying evidence representation, recording, and stopping independently; prior map evidence: `none`.

- **`new_signal` · Structurally bounded exploration · Domain structure can predefine candidate space and stopping.** <!-- timefirst:direction key="structurally-bounded-exploration" state="new_signal" supports="2608.24809" confidence="medium" implication="compare-bounded-and-open-search-under-one-output-contract" timing="radar_published_at" synthesized="2026-08-28T01:27:38Z" prior="none" -->
  Supports: [Crase](#entry-2608.24809); confidence: **medium**; timing basis: `radar_published_at`; Exact synthesis time: `2026-08-28T01:27:38Z` (UTC); Research-design implication (compare bounded and open search under one output contract): hold corpus, seed retriever, generator, output contract, and budget fixed while comparing fixed bounds, adaptive expansion, and open-ended search; prior map evidence: `none`.
