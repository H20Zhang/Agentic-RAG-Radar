...
**Visual status:** legacy Mermaid retired; GPT-image-gen regeneration queued.  
**Grounding brief:** [`assets/visuals/prompts/2608.06305.md`](../assets/visuals/prompts/2608.06305.md)

**What to notice.** The important change is the **retrieval interface**: one opaque top-k call becomes a small operation space whose observations drive the next retrieval decision.

**Compared with.** Dense top-k RAG and the same iterative agent loop with only a top-k retrieval tool.

**Grounding.** Abstract-grounded first pass. The abstract also reports BM25 as statistically indistinguishable from READ, so the visual must not imply that agentic composition itself is proven necessary.
